...
This page is the smallest end-to-end wiring of `@takazudo/zdtp` — just enough code to call `configurePanel({...})`, declare a one-tab manifest, and pop the panel from your browser console. After this, the [Configure Panel reference](/docs/reference/configure-panel) covers every field on `PanelConfig` in depth.
...
Once configured, the package installs three async helpers on `window.<consoleNamespace>` — `showDesignPanel()`, `hideDesignPanel()`, `toggleDesignPanel()`. Calling any of them lazy-imports the panel module and mounts the UI. The package also installs a fixed-name `window.zdtp` global (`zdtp.show()` / `zdtp.hide()` / `zdtp.toggle()`) so you never need to remember your own `consoleNamespace` just to pop the panel open — see [Lifecycle helpers](/docs/reference/configure-panel#lifecycle-helpers) for the full picture.
...
<Warning title="Panel looks unstyled?">
  The panel self-injects its stylesheet when it first mounts — no `import '@takazudo/zdtp/styles'` is needed. If the chrome is missing (transparent background, default page font), the panel module itself failed to load. Check your bundler configuration and confirm the `preact` peer dependency is installed.
</Warning>
