...
styled through the `--astro-*` token namespace.
...
```typescript
// Wiring the prose token namespace for the astro demo
const proseTokens = {
  vspMd:          `var(--astro-vsp-md)`,
  textBody:       `var(--astro-text-body)`,
  leadingRelaxed: `var(--astro-leading-relaxed)`,
  codeBg:         `var(--astro-code-bg)`,
  codeFg:         `var(--astro-code-fg)`,
  fontMono:       `var(--astro-font-mono)`,
} as const;
...
1. Define the token set in `--astro-*` namespace.
...
A blockquote uses a left border coloured by `--astro-color-muted`,
...
Links check `--astro-color-accent` and underline treatment.
...
Heading borders and blockquote accents reuse the shared `--astro-color-fg`,
`--astro-color-muted`, and `--astro-color-accent` tokens — no
