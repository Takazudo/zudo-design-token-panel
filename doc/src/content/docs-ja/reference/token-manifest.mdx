...
`semantic` はカラータブの概念です。`referencesRamps` を使用できるのは、id が正確に `'color'` または `'color-secondary'` である `TabConfig` 内のティアだけであり、それ以外の親タブ id は `assertValidPanelConfig` によって拒否されます。完全な動作契約 — `SemanticValue` ユニオン、単独セマンティックティアのレンダリング方法、タブ間の実例 — については、[カラークラスター](./color-cluster.mdx)を参照してください。
</Info>
