...
| `modes` | No | モード依存行にする light／dark の色ペア。`type.kind` が `'color'` または `'text'` の場合だけ有効で、両側に空でない文字列が必要です。 |
...
`light` と `dark` はどちらも空でない文字列でなければならず、出力時に
`light-dark(light, dark)` へ変換されるため、両側は CSS の色として定義します。
`modes` を指定できるのは item の kind が `color` または `text` の場合だけです。
length、number、select、cursor、content、mask-image の行では拒否されます。
通常の `default` は、モードを選択しない利用側で使われるフォールバックとして残ります。

モード行は左右の側ごとに編集します。通常の単一値入力には変換されず、`light` と
`dark` それぞれにカラーフィールドが表示されます。一方を編集すると、触れていない側を
そのまま保ったまま `light-dark(light, dark)` としてペア全体がコミットされ、Reset で
マニフェストのペアに戻ります。これは汎用タブと Palette の両ビューでの挙動で、Color
タブのパレットスロットに宣言した `modes` はこれまでどおり表示専用です。`readonly` を
省略したモード行は通常の apply と serde の契約に残ります。一方、`readonly` 行は表示専用の
チップ 2 つのまま、apply と import／export の state から除外されます。

`assertValidPanelConfig` は設定境界でこの形を検証します。`modes` がプレーンオブジェクト
でない場合、item の kind が `color` または `text` でない場合、どちらかの側が空でない文字列
でない場合は、フィールドへのパスを含むエラーを返します。たとえば length item では
`modes` が color または text kind でのみ有効だと報告し、`{ light: '', dark: '#000' }` では
`modes.light` が空でない文字列でなければならないと報告します。不正なマニフェストは
パネルがマウントする前に拒否されます。
