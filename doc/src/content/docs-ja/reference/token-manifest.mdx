...
モード行はエディター上で表示専用となり、通常の単一値入力には変換されません。
このエディター無効状態は `readonly: true` とは別のものです。`readonly` を省略した
モード行は通常の apply と serde の契約に残ります。一方、`readonly` 行は表示されますが、
apply と import／export の state から除外されます。
