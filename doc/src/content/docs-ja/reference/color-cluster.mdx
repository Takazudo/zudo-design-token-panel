...
`TierConfig.referencesRamps` を使用できるのは、id が正確に `color` または `color-secondary` である `TabConfig` 内の `semantic: true` ティアだけです。このフィールドは 1 つ以上の*ランプソース*、すなわちセマンティックティアの行が参照できるアイテムを持つティア（任意で別タブ上のもの）を宣言します。
