...
description: "Reference for the zdtp-server bin: every flag, the HTTP endpoint contract, and complete invocation examples."
sidebar_position: 10
---

## Overview

`zdtp-server` is the small Node `http` server that backs the
in-browser **design token panel**. It wraps the framework-agnostic apply
handler shipped from `@takazudo/zdtp/server` in a tiny CLI bin so
...
The bin exposes exactly three endpoint/method pairs (two `/apply` methods and
one health check):

- `POST /apply` — preview or apply a token mutation
...
`POST /apply`, see [Apply pipeline reference](./apply-pipeline.mdx).
</Tip>
...
Suppress the startup banner, the no-allow-origin warning, and per-request
summary logs. Error logs still print.
...
The bin serves exactly three endpoint/method pairs. Any other path returns
...
- Body: JSON payload accepted by the apply handler. Send `dryRun: true` for
  a non-mutating preview with per-file digests and hunks; send those digests
  back as `expectDigests` on the confirming write. See [Apply pipeline
  reference](./apply-pipeline.mdx) for the full payload schema.

**Response:**

- On a dry-run success: `200` with `{ ok: true, dryRun: true, files: [...],
  rejected: [...], rejectedReasons: [...] }`; each file includes `blockKind`,
  a SHA-256 `digest`, `changed`/`unchanged`/`unknown` diagnostics, and one
  hunk per changed CSS variable.
- On a write success: `200` with the JSON body produced by the apply handler.
...
- On a stale preview: `409 { ok: false, reason: "stale-file", files: [...] }`;
  no target is written and the browser should request a fresh preview.
- On other errors: the apply handler's status code (typically `4xx`) and JSON
  body pass through unchanged.

When `--quiet` is not set, a successful preview or apply also writes one
summary line to the bin's stdout. The outcome word is `dry-run` for a preview
and `applied` for a real write:
...
panel mount, see the [Apply pipeline setup recipe](../recipes/apply-pipeline-setup.mdx).
...
- [Apply pipeline reference](./apply-pipeline.mdx) — the
  request/response schema and routing file format consumed by `POST /apply`.
- [Apply pipeline setup recipe](../recipes/apply-pipeline-setup.mdx) —
