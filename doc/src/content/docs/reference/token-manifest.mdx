...
export interface TierItem {
  ...
  type: TierValueKind;
  /** Optional light/dark CSS color pair; makes the row mode-dependent. */
...
| `modes` | no | A light/dark color pair for a mode-dependent row. Valid only for `type.kind: 'color'` or `'text'`; both sides must be non-empty strings. |
...
### `modes` and mode-aware rows

`modes` is a pair of CSS colors:
...
```ts
{
  ...
  default: '#f8fafc',
  type: { kind: 'color' },
  modes: { light: '#f8fafc', dark: '#172033' },
}
```

Both `light` and `dark` must be non-empty strings, and both sides are
documented as CSS colors because the runtime emits the pair as
`light-dark(light, dark)`. `modes` is accepted only when the item kind is
`color` or `text`; it is rejected for length, number, select, cursor, content,
and mask-image rows. The ordinary `default` remains the fallback value used by
consumers that do not select a mode.

Mode rows are edited per side: the panel renders one color field for `light`
and one for `dark` instead of turning the pair into an ordinary single-value
input. Editing one side commits the whole pair as `light-dark(light, dark)`,
copying the untouched side verbatim, and Reset restores the manifest pair. This
applies on generic tabs and on both Palette views; a palette slot declared with
`modes` on the color tab stays display-only. A mode row with `readonly` omitted
remains in the normal apply and serde contract, while `readonly` rows keep the
two display-only chips and are excluded from apply and import/export state.

`assertValidPanelConfig` checks this shape at the configuration boundary. It
reports a path-specific error when `modes` is not a plain object, when the item
kind is not `color` or `text`, or when either side is not a non-empty string.
For example, a length item reports that `modes` is only valid for the color or
text kinds, and `{ light: '', dark: '#000' }` reports that `modes.light` must be
a non-empty string. A malformed manifest is rejected before the panel mounts.
