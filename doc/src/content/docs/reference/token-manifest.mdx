...
Mode rows are display-only in the editor: the panel shows both sides without
turning the pair into an ordinary single-value input. This editor-disabled state
is separate from `readonly: true`. A mode row with `readonly` omitted remains in
the normal apply and serde contract, while `readonly` rows stay visible but are
excluded from apply and import/export state.
