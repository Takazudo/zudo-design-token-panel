...
`semantic` and `referencesRamps` are only meaningful on a tier inside a `'color'` / `'color-secondary'` `TabConfig` (a tier the Color tab's `resolveColorClusterFromTab` bridge reads). For the full behavioral contract — the `SemanticValue` union, how a lone semantic tier renders, and a worked cross-tab example — see [Color cluster § Semantic-only tier](./color-cluster.mdx#semantic-only-tier-semantic-true).
</Info>
