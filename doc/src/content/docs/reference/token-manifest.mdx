...
`TierItem.group` (a sub-heading key inside a tier) was removed in #148 — the panel's [one-tier-one-heading policy](./architecture.mdx) requires each tier to render exactly one section heading. A tier that conceptually needs sub-grouping is split into multiple `TierConfig` entries instead. Example manifests must not reference `group`.
</Note>
