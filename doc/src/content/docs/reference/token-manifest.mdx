...
This page is the type-by-type pin-down of the tier model. If you want to understand the design rationale and the literal / reference tier relationship first, read [The abstract token-tier model](/docs/reference/architecture#the-abstract-token-tier-model).
</Info>
...
| --- | --- | --- | --- |
| `'length'` | Dimension values with a unit | Range slider with a unit suffix (e.g. `rem`, `px`) | `min`, `max`, `step`, `unit` |
| `'number'` | Unitless numeric values | Unitless range slider | `min`, `max`, `step` |
