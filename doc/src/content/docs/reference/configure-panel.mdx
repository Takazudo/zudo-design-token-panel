...
- **Never clobbers a host-defined `window.zdtp`.** If a page already defines its own `window.zdtp`, the install is skipped with a `console.warn` (including when a host happens to pick `consoleNamespace: 'zdtp'` itself).
