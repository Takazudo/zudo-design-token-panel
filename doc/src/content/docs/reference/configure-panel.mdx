...
| `schemaId` | `string` | yes | **Display-only** label surfaced in the Import modal's copy. `serialize()`/`deserialize()` always use the canonical `SCHEMA_V1`/`SCHEMA_V2`/`SCHEMA_V3` constants and never read this field — set it to one of those constants for a truthful label. See [Apply pipeline](/docs/reference/apply-pipeline#export-schema-v2). |
...
| `domTweaker` | `DomTweakerConfig` | no | Opts into the dev-time Tailwind class editor. Omit it to hide the feature — see [DOM Tweaker](/docs/reference/dom-tweaker). |
...
| `autoRememberOnOpen` | `boolean` | no | Whether opening the panel arms owner-mode autoload (`:autoload = 'auto'`). Default `true`. Set `false` for a public site with a visitor-visible panel button — see [Load the panel only for you](/docs/recipes/owner-autoload). |
...
See [DOM Tweaker](/docs/reference/dom-tweaker) for the editing workflow, development-only positioning, `themeCss` bridge, and runtime limitations.
...
- Opening via `zdtp.show()` arms the owner-autoload flag exactly like `showDesignTokenPanel()` — see [Owner-autoload recipe](/docs/recipes/owner-autoload).
