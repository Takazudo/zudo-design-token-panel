...
| `schemaId` | `string` | yes | **Display-only** label returned by `getDesignTokenSchema()`. The built-in import/export UI does not read it; `serialize()`/`deserialize()` always use the canonical `SCHEMA_V1`/`SCHEMA_V2`/`SCHEMA_V3` constants and never read this field. See [Apply pipeline](./apply-pipeline.mdx#export-schema-v2). |
