...
| `schemaId` | `string` | yes | **Display-only** label returned by `getDesignTokenSchema()`. The built-in import/export UI does not read it; `serialize()`/`deserialize()` always use the canonical `SCHEMA_V1`/`SCHEMA_V2`/`SCHEMA_V3` constants and never read this field. See [Apply pipeline](./apply-pipeline.mdx#export-schema-v2-and-v3). |
...
| `dock` | `PanelDockConfig` | no | Controls whether right/bottom docking reserves host space. Default: `{ reflow: 'body-margin' }`. |
...
## `dock` — optional host reflow

The shell supports four presentation modes: `float`, `right`, `bottom`, and
`mini`. The mode and the two dock dimensions are persisted per instance (see
[Storage-key derivation](#storage-key-derivation)). `Alt+1`, `Alt+2`, `Alt+3`,
and `Alt+4` select float, right, bottom, and mini while the full panel is open.
The mini pill expands back to the last full mode.
...
`reflow` applies only to `right` and `bottom` modes:

| Value | Host-document behavior |
| --- | --- |
| `'body-margin'` (default) | Sets `body.style.margin-right` or `body.style.margin-bottom` to the current dock size, while also publishing the corresponding `--zdtp-dock-inset-*` root variable. |
| `'none'` | Publishes the root inset variable but restores the body margin to its exact pre-claim value. |

The claim is per window and per edge. A second panel cannot claim an occupied
edge; it falls back to `float` and persists that fallback. The first claim
captures the previous inline value and priority, and releasing the panel,
changing mode, closing it, or destroying it restores that exact declaration.
The on-page specimen temporarily forces `right` docking and restores the mode
that was active before the specimen was enabled.
...
| `state-v1` | `${storagePrefix}-state` | Pre-v2 flat-state format (Color-only). Migrated to v3 (and from there into v4) on first load, then deleted. |
...
| `dock` | `${storagePrefix}-dock` | Dock mode: `'float'`, `'right'`, `'bottom'`, or `'mini'`. |
| `dock-size` | `${storagePrefix}-dock-size` | Dock dimensions `{ right, bottom }` in pixels; defaults to `{ right: 440, bottom: 340 }`. |
| `density` | `${storagePrefix}-density` | Tab-grid density preference (`0`, `1`, or `2`). |
| `ghost` | `${storagePrefix}-ghost` | Ghost-idle preference (`'1'` when enabled); fading is a presentation preference, not token state. |
...
| `last-applied` | `${storagePrefix}-last-applied` | Flat CSS-variable comparison baseline; after a successful apply the current implementation stores `{}` while retaining only unconfirmed overrides in live state. |
| `visible` | `${storagePrefix}:visible` | Adapter-level visibility-intent flag. |
| `autoload` | `${storagePrefix}:autoload` | Owner autoload provenance: `'1'` (explicit) or `'auto'` (auto-remember). |
...
| `highlight-slots` | `${storagePrefix}-highlight-slots` | Ten highlight slot colors (local storage). |
| `highlight-outline-width` | `${storagePrefix}-highlight-outline-width` | Global highlight outline width in pixels (local storage). |
| `highlight-active` | `${storagePrefix}-highlight-active` | Active CSS-variable → slot map (session storage). |

<Note title="Colon, not dash, for `visible` and `autoload`">
The `visible` and `autoload` keys use a `:` separator; every other derived key
uses `-`. This is a historical artifact preserved for storage-key continuity.
</Note>
...
Applies persisted token overrides to each instance's default `:root` target
(or configured `applySink`) BEFORE any Preact render. Called at adapter module
init (and again on every `astro:page-load`) so the bundle's arrival eliminates
the FOUT on hard navigation. This is a no-op when nothing is persisted; it
swallows errors so corrupt state never blocks the UI thread.
