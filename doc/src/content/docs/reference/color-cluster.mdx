...
Any other owning tab id is rejected at configure time. Each entry names a tier `id` and an optional source `tab` id (omitted `tab` means "this tab"). `assertValidPanelConfig` validates every declared source up front — an unknown tab or tier throws a clear configure-time error, before any row renders.
