...
The target CSS file has **neither** a top-level `:root` block nor a top-level `@theme` block — see [Which CSS blocks get rewritten](#which-css-blocks-get-rewritten) above. A file with only one of the two (e.g. `@theme`-only, no `:root` at all) does NOT 409; it applies normally against whichever block exists.
