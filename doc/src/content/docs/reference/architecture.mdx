...
1. **Panel UI** — the Preact island that renders the side panel, owns tab and control state, and writes overrides through the configured `applySink` (or the default `:root` sink). Lives entirely in the browser.
2. **Host-adapter** — the thin shim a host imports as a side effect. It reads the inline JSON config, calls `configurePanel(...)`, installs `window.<consoleNamespace>.*` plus the fixed-name `window.zdtp.{show,hide,toggle}()` alias, and gates the lazy-load of the panel module.
...
At apply time, the resolver emits `--myapp-spacing-md: 1.25rem` (or whatever the current override is) directly to the configured `applySink`, or to `:root` when no sink is configured.
...
The color tab carries extra non-item metadata (color-picker configuration,
color schemes, base roles, and optional secondary-cluster data) in
`colorExtras` on the `TabConfig`. This metadata does not map to individual
TierItems; it drives the color tab's UI chrome and scheme switching.
...
## Font specimen and host-page rendering

The font tab can mark tiers with `preview` metadata. `size` tiers are sorted
by resolved pixel value and show the configured specimen text; a
`line-height` tier repeats that text at a base size. When `previewBase` names
a CSS variable, the renderer first resolves a matching manifest item and then
the document-root computed value; otherwise it uses the nearest resolved size
to 16px (or 16px when no size can be resolved). The first item in a `family`
or `weight` tier supplies the font style used by every sample. `bar`,
`radius`, and `duration` render compact glyphs in ordinary token rows.

The Font tab's **Render on page** option creates a
`body`-first `div.tokenpanel-on-page-specimen[data-zdtp-specimen]` portal.
The portal deliberately inherits the host page's font and foreground color;
it is not panel chrome and is excluded from token/highlight/picker scans. It
contains a level-three heading and one **Scale** section with one row for every
`preview: 'size'` item, plus a **Line height** section for each line-height
tier. Each line-height sample exposes a leading guide through
`--tokenpanel-specimen-line` and uses
the toolbar width (240–720px, default 420px).

While this portal is enabled, the panel temporarily claims the `right` dock so
the page specimen has room. The panel remembers the prior mode in memory,
never as a second storage key, and restores it when the option is disabled,
the panel closes, navigation unmounts the shell, or a dock claim is lost.

## Dock reflow and host mutations

`dock.reflow` controls the host-document side effect for a `right` or `bottom`
dock. The default `'body-margin'` sets `body.style.margin-right` or
`body.style.margin-bottom` to the current dock dimension and publishes the
same pixel value as `--zdtp-dock-inset-right` or `--zdtp-dock-inset-bottom` on
the document root. `'none'` publishes the root inset but restores the body's
exact pre-claim value. The registry permits one owner per edge across bundles;
another owner falls back to `float`. The first claim snapshots both inline
values and priorities, and every release restores them exactly.

`float` is draggable and `right`/`bottom` are resizable from their inner edge.
`mini` is a compact pill that expands back to the last full mode. Dock mode,
dimensions, and the ghost-idle preference are independent of token state and
are persisted under the instance's storage prefix.

## One transaction path for token state

All mounted-panel token mutations go through `commitTweakState(reason,
updater, options)`. The transaction applies the next state, saves the
persisted envelope, updates component state, and then records history in that
fixed order. It is used by single-row edits, bulk edits, element-inspect
adjust/revert, imports, snapshot restores, and undo/redo; bulk operations
therefore create one history entry even when they touch many rows. History is
in memory only. The v4 envelope and the A/B snapshot slots are persisted
separately, while `last-applied` stores the flat comparison baseline used by
the changed-state indicator. After a successful disk apply it is reset to
`{}`, and only confirmed-written variables are reconciled from live state.
...
On every input change or color pick, the resolver calls `resolveTierItemValue(tab, tierId, itemId, overrides)` and then `emitTierItemCssValue(resolved)` to get the final CSS string. The result is written immediately to the default `:root` target via `document.documentElement.style.setProperty(cssVar, value)`, or to the configured `applySink` for that instance.
...
State is persisted to `localStorage` under `${storagePrefix}-state-v4`. On first load after an upgrade the panel automatically migrates any existing v3 state into the v4 envelope (the legacy v3 key is left in place, not deleted); v2 and v1 are normalised to v3, then copied into v4 in the same pass. No user action is required.

| Storage key | Contents | Lifecycle |
| --- | --- | --- |
| `${storagePrefix}-state-v4` | Current unified envelope. Same slices as v3, except `color` (and any secondary cluster slice) is keyed by active (scheme, mode) identity — one slot per identity — rather than a single flat object. | Live; written on every change. See [Per-scheme color persistence](./configure-panel.mdx#storage-key-derivation-per-scheme-color-persistence-v4-envelope). |
| Previous v3 key | v3 envelope (flat, single-slot `color`) — migrated into v4 on first load. Left in place (not deleted) so a downgrade can still read it. | Transitional — see [Storage-key derivation](./configure-panel.mdx#storage-key-derivation). |
| Previous v2 key | v2 envelope — migrated to v3 (and from there into v4) on first load, then deleted. | Transitional. |
| `${storagePrefix}-state` | Pre-v2 flat state (color-only). Migrated to v3 (and from there into v4) on first load, then deleted. | Legacy. |

### JSON export and import

The **Export** button emits a JSON file whose `$schema` field identifies it as `SCHEMA_V2` or, when the state holds a semantic mapping v2's number-only leaf can't represent (a `{ literal }` / `{ literal: { light, dark } }` / `{ ref }` value), the superset `SCHEMA_V3`. The **Import** modal accepts `SCHEMA_V1`, `SCHEMA_V2`, and `SCHEMA_V3` payloads and normalises them internally before applying. `PanelConfig.schemaId` is a display-only label returned by `getDesignTokenSchema()`; the built-in UI does not read it, and it is never read by the serializer or validator; see [`PanelConfig` interface](./configure-panel.mdx#panelconfig-interface). The export schema version is independent of the localStorage schema version — bumps to one do not imply a bump to the other.

### Optional disk rewrite

A tweak never leaves the browser — the active CSS target updates synchronously on every change and the state is persisted to `localStorage` under `${storagePrefix}-state-v4`. The bin server only runs when the host passes `applyEndpoint` + `applyRouting` on `PanelConfig`, and even then only activates when the user clicks Apply.
...
- [Configure Panel reference](./configure-panel.mdx) — full `PanelConfig` field reference.
- [Apply pipeline reference](./apply-pipeline.mdx) — CLI flags, security model, routing JSON shape.
- [Color cluster reference](./color-cluster.mdx) — `colorExtras` and `ColorClusterExtras` in depth.
