...
State is persisted to `localStorage` under `${storagePrefix}-state-v4`. On first load after an upgrade the panel automatically migrates any existing v3 state into the v4 envelope (the legacy v3 key is left in place, not deleted); v2 is promoted to v3 and v1 to v2 in the same pass. No user action is required.
...
| `${storagePrefix}-state` | Pre-v2 flat state (color-only). Migrated to v2 (and from there into v4) on first load, then deleted. | Legacy. |

### JSON export and import

The **Export** button emits a JSON file whose `$schema` field identifies it as `SCHEMA_V2` or, when the state holds a semantic mapping v2's number-only leaf can't represent (a `{ literal }` / `{ literal: { light, dark } }` / `{ ref }` value), the superset `SCHEMA_V3`. The **Import** modal accepts `SCHEMA_V1`, `SCHEMA_V2`, and `SCHEMA_V3` payloads and normalises them internally before applying. `PanelConfig.schemaId` is a display-only label surfaced in the Import modal's copy — it is never read by the serializer or the validator; see [`PanelConfig` interface](./configure-panel.mdx#panelconfig-interface). The export schema version is independent of the localStorage schema version — bumps to one do not imply a bump to the other.

### Optional disk rewrite

A tweak never leaves the browser — `:root` updates synchronously on every change and the state is persisted to `localStorage` under `${storagePrefix}-state-v4`. The bin server only runs when the host passes `applyEndpoint` + `applyRouting` on `PanelConfig`, and even then only activates when the user clicks Apply.
