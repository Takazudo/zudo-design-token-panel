...
| --- | --- | --- |
| `${storagePrefix}-state-v4` | Current unified envelope. Same slices as v3, except `color` (and any secondary cluster slice) is keyed by active (scheme, mode) identity — one slot per identity — rather than a single flat object. | Live; written on every change. See [Per-scheme color persistence](/docs/reference/configure-panel#per-scheme-color-persistence-v4-envelope). |
| Previous v3 key | v3 envelope (flat, single-slot `color`) — migrated into v4 on first load. Left in place (not deleted) so a downgrade can still read it. | Transitional — see [Storage-key derivation](/docs/reference/configure-panel#storage-key-derivation). |
