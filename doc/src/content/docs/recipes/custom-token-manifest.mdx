...
[Token manifest reference](../reference/token-manifest.mdx).
...
[Multi-tier tokens](./multi-tier-tokens.mdx) recipe for a full
worked example.
...
- [Multi-tier tokens](./multi-tier-tokens.mdx) — 2-tier raw + semantic
  setup with `referencesTier` and `var()` resolution.
- [Token manifest reference](../reference/token-manifest.mdx) — full `TierItem`,
  `TierConfig`, `TabConfig`, and `TierValueKind` interfaces.
- [`configurePanel` reference](../reference/configure-panel.mdx) — where `tabs`
  plugs in.
- [Apply pipeline setup](./apply-pipeline-setup.mdx) — wiring
