...
description: "Wire the zdtp-server bin into a non-Astro host's dev script — CORS, --write-root sandboxing, and the routing JSON."
sidebar_position: 4
---

The Apply button POSTs a flat CSS-var diff to the host's apply endpoint,
which routes the diff to the **bin server** (`zdtp-server`),
...
[Apply pipeline reference](../reference/apply-pipeline.mdx).
...
The package ships an executable named `zdtp-server`. Verify
the install with:
...
The routing JSON only says WHICH FILE a token family lands in — it says
nothing about where inside that file the value gets rewritten. The rewriter
itself scans the first top-level `:root { ... }` block AND the first
top-level `@theme { ... }` block of the routed file (Tailwind v4's token
scope), so a token declared inside either one is reachable; see
[Which CSS blocks get rewritten](../reference/apply-pipeline.mdx#apply-to-disk-which-css-blocks-get-rewritten)
for the full scanning rules and a Tailwind v4 worked example.
...
- **`--write-root ./src/styles`** — required. This is the narrow directory
  tree the bin is allowed to write into; keep it limited to the files the
  panel edits so a typo or path-escape attempt cannot clobber other files in
  your repo.
- **`--root <dir>`** — optional. This is the CWD/resolution base for relative
  `--routing` and `--write-root` paths, and defaults to `process.cwd()`.
...
[apply pipeline reference](../reference/apply-pipeline.mdx) for the full
...
- [Apply pipeline reference](../reference/apply-pipeline.mdx) — request /
  response envelopes, error codes, native-implementation guidance.
- [`configurePanel` reference](../reference/configure-panel.mdx) —
  `applyEndpoint` and `applyRouting` fields.
- [Token manifest reference](../reference/token-manifest.mdx) — which
