...
[Color cluster reference](../reference/color-cluster.mdx).
...
See [`configurePanel` reference](../reference/configure-panel.mdx) for the full
...
[secondary cluster recipe](./secondary-cluster-or-disable.mdx) and
the [Color cluster reference](../reference/color-cluster.mdx) section on
`panelSettings.colorMode`.

## Related

- [Color cluster reference](../reference/color-cluster.mdx) — full type
  contract, `ColorClusterExtras`, and the internal bridge semantics.
- [Panel CSS tokens](../reference/panel-css-tokens.mdx) — overriding the
  panel's chrome colors (separate from your app's color cluster).
- [Lazy color presets](./lazy-color-presets.mdx) — keep a large
