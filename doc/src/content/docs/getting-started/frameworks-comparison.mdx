...
The panel's public surface is a single `configurePanel({...})` call plus a Preact-rendered shell, so the wiring shape is the same across hosts. The only differences are *who* calls `configurePanel`, *how* the host interacts with view-transitions, and *how* the apply proxy is registered. The styles import is not a differentiator — the panel self-injects its stylesheet on first mount (no consumer CSS import required in any framework).

This page is the side-by-side overview. For full code, see the [Examples](./examples.mdx) page — each framework's live demo and source repo are linked there.

## Comparison

| Step | Astro | Vite + React | Next.js (App Router) | zfb | zfb + Tailwind v4 |
| --- | --- | --- | --- | --- | --- |
| Install package | `pnpm add @takazudo/zdtp preact` | same | same | same | same + `tailwindcss` |
...
| Styles import | not required (self-injected) | not required (self-injected) | not required (self-injected) | not required (self-injected) | not required (self-injected); tokens also registered via `@theme` block |
...
- The fixed-name `window.zdtp.{show,hide,toggle}()` global — a `consoleNamespace`-agnostic alias installed identically by every host, layered on top of the namespaced console API.
...
Open devtools, run `window.myapp.toggleDesignPanel()` (or the namespace-agnostic `zdtp.toggle()`), and the panel mounts.
...
- For per-field detail on `PanelConfig`, see the [Configure Panel reference](../reference/configure-panel.mdx).
- For the apply-pipeline bin (`zdtp-server`) and routing JSON, see the [CLI reference](../reference/cli-zdtp-server.mdx).
- For the smallest possible end-to-end wiring, see [Quickstart](./quickstart.mdx).
