...
After zfb fix [#229](https://github.com/Takazudo/zudo-front-builder/issues/229), `devMiddleware`-registered paths are scoped under `base`, so the bare path resolves to 405. This is a configuration requirement for this deployment, not a zfb bug. See [`PROBE-REPORT.md`](https://github.com/Takazudo/zudo-design-token-panel-example-zfb/blob/main/PROBE-REPORT.md) for the full rationale.
