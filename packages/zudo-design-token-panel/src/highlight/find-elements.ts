...
export interface FindElementsOptions {
  ...
  kind?: 'color' | 'length' | 'number' | 'text' | 'easing' | 'cursor' | 'content' | 'mask-image';
  /**
   * Probe mode.
...
// Time-typed properties (transition-duration, animation-duration) need their own
 ...
 *   6. quoted string → 'content'
 *   7. url() → warning + fall through to 'text'
...
function detectKind(
  cssVar: string,
  warnings: string[],
): 'color' | 'length' | 'number' | 'text' | 'easing' | 'content' {
 ...
 *   - `kind`: explicit token type (`'color' | 'length' | 'number' | 'text' | 'easing' | 'cursor' | 'content' | 'mask-image'`).
 ...
 *     animation-timing-function). `cursor` covers the cursor property using
 *     keyword sentinels (Chrome drops url() from cursor computed style).
 *     `content` covers the content property using quoted-string sentinels.
 *     `mask-image` covers the mask-image property using url() sentinels.
...
export function findElementsUsingToken(
  ...
  const kind: 'color' | 'length' | 'number' | 'text' | 'easing' | 'cursor' | 'content' | 'mask-image' =
