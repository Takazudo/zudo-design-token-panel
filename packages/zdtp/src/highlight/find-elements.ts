 ...
 * portal mounts, including element inspect). Exported so picker features can
 * skip the same surfaces when resolving the element under the cursor.
 */
export const PANEL_EXCLUSION_SELECTOR =
  `.tokenpanel-shell, [data-design-token-panel-modal], #${HIGHLIGHT_PORTAL_MOUNT_ID}, #${ELPATH_PORTAL_MOUNT_ID}, #${DOM_TWEAKER_PORTAL_MOUNT_ID}, #${ELEMENT_INSPECT_PORTAL_MOUNT_ID}`;
