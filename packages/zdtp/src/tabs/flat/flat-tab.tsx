...
export default function FlatTab({
  ...
  const visibleEntriesByTier = useMemo(() => new Map(
    [...entriesByTier].map(([tierId, entries]) => [
      tierId,
      entries.filter((entry) => contributions.every((contribution) => contribution.filter?.(entry) !== false)),
    ]),
  ), [contributions, entriesByTier]);
  ...
  const renderedContributions = useMemo(
    () => bulkContribution ? [bulkContribution, ...contributions] : contributions,
    [bulkContribution, contributions],
  );
