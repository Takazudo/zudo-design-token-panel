...
export default function SizeTab({ tab, state, persistSize, onBulkApply }: SizeTabProps) {
  ...
  return <FlatTab tab={tab} getValue={getValue} setValue={setValue} deleteValue={deleteValue}
    overrides={overrides} contributions={[PREVIEW_GLYPH]} sectionTestId={(tier) => `size-tier-${tier.id}`} onBulkApply={onBulkApply} actions={(
