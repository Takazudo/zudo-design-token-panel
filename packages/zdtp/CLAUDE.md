...
Hostile-host CSS isolation is verified by the automated Level-5 browser gate at `packages/zdtp/src/__tests__/hostile-host-isolation.browser.test.tsx` (F33 + F24), which runs in CI via Playwright Chromium on every PR. The manual DevTools harness page this test was converted from (`doc/src/content/docs/internal/hostile-host.mdx`, `unlisted: true`, written for the Panel Hardening epic #145) was dropped in the zudo-doc 4.x docs migration — the `internal/` docs section no longer exists, so the automated test below is now the sole source of truth for this scenario.

### What it covers

- The complete hostile CSS block (h1–h6, button, p/ul/li, pre/code, svg/path, universal box-shadow) used to verify CSS isolation.
- F24: a CSS transform-ancestor scenario verifying the panel's `position:fixed` bounding box stays viewport-relative rather than resolving against a transformed sibling.
- Static-grep findings are enforced separately by `manifest-cascade-verification.test.ts` (Invariant D, below).

### Running the check manually (optional, ad hoc debugging)

1. Start any example app from its sibling repo under `$HOME/repos/zdtp-ex/` (Astro example recommended): `cd $HOME/repos/zdtp-ex/zudo-design-token-panel-example-astro && pnpm dev`. The full list of external example repos is in `README.md` §15.
2. Reproduce the hostile `<style>` injection block from `hostile-host-isolation.browser.test.tsx` in DevTools console.
